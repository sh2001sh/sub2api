export default {
  ...
  keys: {
    ...
    modelQuotaSection: '单模型额度限制',
    modelQuotaHint: '为指定请求模型设置独立的 USD 限额。留空表示不启用模型级额度限制。',
    modelQuotaAdd: '新增模型限额',
    modelQuotaEmpty: '还没有配置模型限额规则。',
    modelQuotaModelLabel: '模型',
    modelQuotaModelPlaceholder: '例如 gpt-4.1-mini',
    modelQuotaLimitLabel: '限额 (USD)',
    modelQuotaLimitPlaceholder: '0.00',
    modelQuotaUsedLabel: '已用',
    modelQuotaConfiguredCount: '已配置 {count} 条模型限额规则',
    modelQuotaValidationIncomplete: '每条模型限额都必须同时填写模型名和限额。',
    modelQuotaValidationLimit: '模型限额必须大于 0。',
    modelQuotaValidationDuplicate: '模型限额重复：{model}',
    resetModelQuotaUsage: '重置模型限额用量',
    resetModelQuotaTitle: '确认重置模型限额',
    resetModelQuotaConfirmMessage: '确定要重置密钥 "{name}" 的所有模型限额用量吗？此操作不可撤销。',
    modelQuotaResetSuccess: '模型限额用量已重置',
    failedToResetModelQuota: '重置模型限额用量失败',
  ...
  admin: {
    ...
    accounts: {
      ...
      cpaImport: '导入 CPA',
      cpaImportTitle: '导入 CPA 认证文件',
      cpaImportHint: '上传 CPA 根目录，或上传 auths/*.json 与 config/config.yaml，系统会自动转换成 sub2api 原生导入数据。',
      cpaImportWarning: '该导入会创建新账号。CPA config 中的旧 API Key 会单独保留为待手工处理列表；停用账号和 service_account 账号会被有意跳过。',
      cpaImportFiles: 'CPA 文件',
      cpaImportSupportedFiles: '支持 auths/*.json 与 config/config.yaml',
      cpaImportChooseFolder: '选择目录',
      cpaImportChooseFiles: '选择文件',
      cpaImportSelectFiles: '请选择 CPA 目录或文件',
      cpaImportSelectedFiles: '已选择 {count} 个文件',
      cpaImportSkipDefaultGroupBind: '导入后不自动绑定平台默认分组',
      cpaImportButton: '转换并导入',
      cpaImporting: '导入中...',
      cpaImportFailed: 'CPA 导入失败',
      cpaImportSuccess: 'CPA 导入完成：创建账号 {account_created}，跳过 {accounts_skipped}，保留旧 Key {api_keys_preserved}',
      cpaImportCompletedWithErrors: 'CPA 导入完成但有问题：账号失败 {account_failed}，代理失败 {proxy_failed}，跳过 {accounts_skipped}',
      cpaImportResult: 'CPA 导入结果',
      cpaImportSeen: '扫描账号',
      cpaImportConverted: '导入成功',
      cpaImportSkipped: '已跳过',
      cpaImportKeysPreserved: '保留旧 Key',
      cpaImportWarnings: '转换警告',
      cpaImportSkippedDetails: '跳过详情',
      cpaImportPreservedKeys: '保留的旧 API Key',
      cpaImportResultSummary: '创建代理 {proxy_created}，复用代理 {proxy_reused}；创建账号 {account_created}；跳过 {accounts_skipped}；保留旧 Key {api_keys_preserved}',
